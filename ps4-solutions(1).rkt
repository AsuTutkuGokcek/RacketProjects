...
(define (empty-env? env)
  (cond ((null? env) (display "no binding found"))
        ((eq? (car env) 'empty-env) #t)
        (else #f)))
...
(define (apply-rib-env env search-var)
  (letrec ((rec-search (lambda (saved-vars saved-vals search-var)
                         (cond ((null? saved-vars) '())
                               ((eqv? (car saved-vars) search-var) (car saved-vals))
                               (else (rec-search (cdr saved-vars) (cdr saved-vals) search-var))))))
    (cond
      ...
      ((eqv? (car env) 'extend-rib-env)
       (let* ((saved-vars (caadr env))
              ...
              (saved-env (caddr env))
              (result (rec-search saved-vars saved-vals search-var)))
         (if (null? result)      
