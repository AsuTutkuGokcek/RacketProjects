  ...
  (define (apply-env env search-var)
    (cond
      ...
      ((eqv? (car env) 'extend-env)
         (let ((saved-pair (cadr env))
               (saved-env (caddr env))
               )
           (if (eqv? search-var (car saved-pair))
               (cadr saved-pair)
...
;; Part B  
    ...
    (define (extend-env* var-list val-list env)
      (if (or (null? var-list) (null? val-list))
       env
...
(define (extend-rib-env* var-list val-list env)
  ...
  (define (apply-rib-env env search-var)
      (if (eqv? (car env) 'extend-rib-env)
         (let ((saved-vars (caadr env))
               (saved-vals (cadadr env)))
           (if (null? (apply-helper saved-vars saved-vals search-var))
               (apply-rib-env (caddr env) search-var)
               (apply-helper saved-vars saved-vals search-var)))
         '()))

     (define (apply-helper saved-vars saved-vals search-var)
      (if (null? saved-vars)
          '()
      (if (eqv? search-var (car saved-vars))
          (car saved-vals)
          (apply-helper (cdr saved-vars) (cdr saved-vals) search-var))))
  ...
  (define (bintree-to-list tree)
         (cases bintree tree
           ...
           (interior-node (key left right) (list 'interior-node
                                                   key  (bintree-to-list left)
                                                                   (bintree-to-list right)))))

  (define (sum-leafs tree)
    (cases bintree tree
      (leaf-node (num) num)
      (interior-node (key left right) (+ (sum-leafs left) (sum-leafs right)))))
