(module ps3 mzscheme
  ...
  ;
  ;In Unary Representation, natural numbers are represented in equal amount of #t statements
  ;in a list.
  ;In BigNum Representation, natural numbers are represented in a list of some smaller numbers
  ;that are calculated based on a given base N which add up to the natural number itself. The
  ;numbers that go into the list are calculated based on binary respresentation and written
  ;starting from the least-significant bigit.
  ;
  ;
  ;
  ;; PROBLEM 1 Part B
  ...
  (define predecessor-u
    (lambda (n) (cond ((null? n) '())
                      ((not (null? n)) cdr n)
                      (else 'error-only-positive-numbers'))))
